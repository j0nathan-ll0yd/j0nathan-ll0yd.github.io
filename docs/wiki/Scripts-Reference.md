...
| `test:visual:docker` | Runs `scripts/run-in-docker.sh playwright.config.ts` inside `mcr.microsoft.com/playwright:v{VERSION}-noble`, **arm64-native** under Docker Desktop's Apple Virtualization Framework. **Canonical local run** — the self-hosted CI runner (`ci-runners-private/images/runner-playwright`) is `FROM` the same upstream tag, so PNG bytes match CI exactly. | Developer before any push (enforced by `.husky/pre-push`); reusable workflow `j0nathan-ll0yd/ci-runners-private/.github/workflows/web-visual-tests.yml` (dispatched by `visual-tests.yml`). |
| `test:visual:update:docker` | Same as `test:visual:docker` but passes `--update-snapshots`. The sanctioned local path to regenerate PNG baselines after intentional visual changes; commit the resulting PNGs. Equivalent CI path: dispatch `visual-tests.yml` with `update_snapshots=true`. | Developer after intentional visual changes; `web-visual-tests.yml` `update_snapshots=true` mode. |
...
- **Baseline regeneration** must go through `test:visual:update:docker` (local, arm64-native) or `visual-tests.yml` with `update_snapshots=true` (CI, self-hosted arm64). The two paths share the same Playwright noble base image, so their bytes are byte-identical.
