...
| `prebuild` | npm lifecycle hook — runs automatically before `build`. Four steps: `generate:webmcp` → DS schemas self-validation (`@lifegames/schemas/scripts/validate.ts`) → `audit:fixtures` → `audit:inline-scripts`. Any failure halts the build. | npm lifecycle — automatic before `npm run build`. Never run directly. |
| `audit:fixtures` | Runs `scripts/audit-fixtures.mjs` — Invariant I2 gate: fails the build if any consumer-side fixture JSON appears under `data/`, `test/fixtures/`, or `src/**/fixtures/`. Fixtures are DS-owned (`@lifegames/fixtures`). | `prebuild` (automatic); developer manually when verifying no local fixtures crept in. |
...
| `build` | `astro build` — compiles `src/` into `dist/`; the SSR shell comes from `@lifegames/fixtures` via `loadDashboardData()`. Preceded by `prebuild`, followed by `postbuild`. | `deploy.yml` (`npm run build` step); `playwright.config.ts` `webServer.command`; `vitest.build.config.ts` global setup. |
