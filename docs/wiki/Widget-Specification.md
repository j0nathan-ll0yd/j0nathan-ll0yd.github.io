...
Data flows through three coordinated pipelines:

```text
BUILD TIME (deterministic DS shell and visual tests):
...
PUBLIC HTML (Cloudflare Pages edge):
  CloudFront raw exports → portal-contract JSON Schema + Ajv → per-domain provenance/freshness → semantic live snapshot
