...
They live in `public/.well-known/` and Astro copies them to `dist/` at build time, but
MOST OF THEM ARE GENERATED, NOT AUTHORED: `scripts/generate-webmcp.mjs` writes
`mcp/server-card.json`, `agent-skills/index.json`, `agent-skills/portfolio-expert/SKILL.md`
and `ai-catalog.json` during `prebuild`. Do not hand-edit those four -- the next build
overwrites the edit. Only `api-catalog`, `security.txt` and `webfinger` are hand-authored.
(This section called all of them "static files" until atlas decision 0142 phase 7, which
contradicted `docs/discovery-surface.md` and invited exactly that lost edit.)

### Discovery Files (in this repo)

| File                                                        | Spec                          | Purpose                                           | Authored or generated             |
| ----------------------------------------------------------- | ----------------------------- | ------------------------------------------------- | --------------------------------- |
| `public/.well-known/api-catalog`                            | RFC 9727 (linkset JSON)       | Advertises CloudFront data API to agents          | authored                          |
| `public/.well-known/mcp/server-card.json`                   | MCP SEP-2127                  | Declares read-only data resources for MCP clients | generated by `generate-webmcp.mjs` |
| `public/.well-known/agent-skills/index.json`                | Agent Skills Discovery v0.2.0 | Skills discovery index                            | generated by `generate-webmcp.mjs` |
| `public/.well-known/agent-skills/portfolio-expert/SKILL.md` | agentskills.io                | Curated portfolio context for agents              | generated by `generate-webmcp.mjs` |
| `public/.well-known/ai-catalog.json`                        | ARD `specVersion` 1.0         | Catalogs the MCP and Agent Skills resources       | generated by `generate-webmcp.mjs` |
