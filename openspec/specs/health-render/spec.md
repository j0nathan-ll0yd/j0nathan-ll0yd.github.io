...
- **WHEN** each is rendered in turn
- **THEN** the Heart Rate card SHALL show its paused block with its data wrapper hidden for each,
  reading "Watch off" for the first and "Watch charging" for the second; for the off-wrist export
  the Movement card SHALL likewise show its paused block with its data wrapper hidden, and for the
  charging export its paused label SHALL read "Watch charging"
