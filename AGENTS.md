...
- **Estate contracts are consumed, never vendored** -- atlas decisions 0079 item 4 wave 2b, 0080. The `openspec-covers` rule (`scripts/openspec-covers.mjs`, the blocking `covers-conformance` CI gate) and the `llms-structure` rule (`audits/checks/b2-validate-llms-txt.mjs`) both import `@j0nathan-ll0yd/estate-contracts`. Never re-vendor either one, and never hand-write a `.sha256` sidecar for them. Three consequences:
  - **The dependency is EXACT-pinned (`0.8.1`), not a caret.** A `SPEC_VERSION` is a rule version, and while the package is `0.x` the minor field is the breaking bump, so a caret would silently admit a different rule. Moving the pin is an estate-atomic operation: read the tier README, reconcile `openspec/`, and move `EXPECTED_SPEC_VERSION` in `scripts/openspec-covers.mjs` and in both integrity tests in the same change when a rule version changes. A release that leaves the RULE alone moves the pin alone: `0.5.0` added the decision-0099 llms.txt codec (`parseLlmsTxt`, `encodeLlmsTxt`, `decodeLlmsTxt`), `0.6.0` changed that codec's canonical encode form (a run of consecutive bullet-shaped prose lines now renders contiguous), `0.7.0` (decision 0103) moved the codec's model and encode guards onto Zod while preserving behavior, and `0.8.0` (decision 0110) touched only the `llms-assurance` tier, but `checkLlmsStructure` is untouched by all four, so `LLMS_STRUCTURE_SPEC_VERSION` stays 3 and `COVERS_SPEC_VERSION` stays 4. What DOES move on any such release is the shipped `reference.mjs` and its `.sha256` sidecar -- the integrity test reads the shipped sidecar so it self-updates, but any digest quoted in `openspec/` is a hand-maintained citation and must be re-read from `node_modules/@j0nathan-ll0yd/estate-contracts/llms-structure/reference.mjs.sha256` in the same change. `0.8.0` is the one release so far that moved NEITHER rule tier's bytes: `llms-structure/reference.mjs` and `openspec-covers/reference.mjs` are byte-identical to `0.7.0` (sidecars `149ea49a...` and `13e7fc5a...` unchanged), so the digest quoted in `openspec/specs/llms-txt/spec.md` needed no edit on that bump.
  - **`0.8.0` (decision 0110) removed the provenance record; the B2 spoke-evidence contract this repo emits against is unchanged.** Gone: `PROVENANCE_KINDS`, `validateProvenance`, `assertProvenance`, and the `./llms-assurance/provenance.schema.json` subpath. Arrived: `SERVED_VERIFICATION_SPEC_VERSION`, `SERVED_VERIFICATION_KIND`, `SERVED_VERIFICATION_VERDICTS`, `validateServedVerification`, `assertServedVerification` -- Atlas check A20 is that record's only writer and reader. This repo imported none of the removed names, so the bump was a pin move with no code change. `spoke-evidence.schema.json` and `freshness-config.v1.json` are byte-identical to `0.7.0`, and `validateSpokeEvidence`'s expected B2 `source.repository` is still `j0nathan-ll0yd.github.io`.
  - **The shared `runSource` in `0.8.0` is a private validator, not an adoptable builder.** Atlas PR #268 lifted the GitHub Actions run block out of `validateSpokeEvidence`'s `$.source` so a served verification's `$.verifiedBy` could ask the same rule. `runSource`, `RUN_SOURCE_FIELDS`, and `RUN_SOURCE_REQUIRED` are module-private `const`s in `llms-assurance/reference.mjs` -- not exported, not reachable through any `exports` subpath -- and the refactor is behavior-preserving (same fields, same required set, same messages). `checkedSource` in `audits/lib/llms-spoke-evidence.ts` is a BUILDER that constructs and type-narrows the envelope, a different job from a validator that pushes error strings, and the validation half is already delegated: `audits/checks/b2-check-llms-coherence.mjs` runs the published `assertSpokeEvidence` over every envelope before writing it. So there is nothing to adopt here today. Promoting a shared run-source builder is atlas decision 0111 phase 3 and needs an EXPORTED builder in the contract first.
