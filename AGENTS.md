...
- **Estate contracts are consumed, never vendored** -- atlas decisions 0079 item 4 wave 2b, 0080. The `openspec-covers` rule (`scripts/openspec-covers.mjs`, the blocking `covers-conformance` CI gate) and the `llms-structure` rule (`audits/checks/b2-validate-llms-txt.mjs`) both import `@j0nathan-ll0yd/estate-contracts`. Never re-vendor either one, and never hand-write a `.sha256` sidecar for them. Six consequences:
  - **The dependency is EXACT-pinned (`0.9.0`), not a caret.** A `SPEC_VERSION` is a rule version, and while the package is `0.x` the minor field is the breaking bump, so a caret would silently admit a different rule. Moving the pin is an estate-atomic operation: read the tier README (in the atlas `contracts/` tree -- tier READMEs stopped shipping in the tarball at `0.9.0`, decision 0113 R2b), reconcile `openspec/`, and move `EXPECTED_SPEC_VERSION` in `scripts/openspec-covers.mjs` and in both integrity tests in the same change when a rule version changes. A release that leaves the RULE alone moves the pin alone: `0.5.0` added the decision-0099 llms.txt codec (`parseLlmsTxt`, `encodeLlmsTxt`, `decodeLlmsTxt`), `0.6.0` changed that codec's canonical encode form (a run of consecutive bullet-shaped prose lines now renders contiguous), `0.7.0` (decision 0103) moved the codec's model and encode guards onto Zod while preserving behavior, `0.8.0` (decision 0110) touched only the `llms-assurance` tier, and `0.9.0` (the consolidated 0110/0111/0113/0117 release, atlas PR #284) touched only `llms-assurance` and packaging, but `checkLlmsStructure` is untouched by all five, so `LLMS_STRUCTURE_SPEC_VERSION` stays 3 and `COVERS_SPEC_VERSION` stays 4. What DOES move on any such release is the shipped `reference.mjs` and its `.sha256` sidecar -- the integrity test reads the shipped sidecar so it self-updates, but any digest quoted in `openspec/` is a hand-maintained citation and must be re-read from `node_modules/@j0nathan-ll0yd/estate-contracts/llms-structure/reference.mjs.sha256` in the same change. `0.8.0` and `0.9.0` are the two releases so far that moved NEITHER rule tier's bytes: `llms-structure/reference.mjs` and `openspec-covers/reference.mjs` are byte-identical to `0.7.0` (sidecars `149ea49a...` and `13e7fc5a...` unchanged, re-read at `0.9.0`), so the digest quoted in `openspec/specs/llms-txt/spec.md` needed no edit on either bump.
  - **`0.8.0` (decision 0110) removed the provenance record; the B2 spoke-evidence contract this repo emits against is unchanged.** Gone: `PROVENANCE_KINDS`, `validateProvenance`, `assertProvenance`, and the `./llms-assurance/provenance.schema.json` subpath. Arrived: `SERVED_VERIFICATION_SPEC_VERSION`, `SERVED_VERIFICATION_KIND`, `SERVED_VERIFICATION_VERDICTS`, `validateServedVerification`, `assertServedVerification` -- Atlas check A20 is that record's only writer and reader. This repo imported none of the removed names, so the bump was a pin move with no code change. `spoke-evidence.schema.json` is byte-identical to `0.7.0`, and `validateSpokeEvidence`'s expected B2 `source.repository` is still `j0nathan-ll0yd.github.io`. `0.9.0` (decision 0110 action 3) then collapsed the freshness config: `freshness-config.v1.json`, its schema, and both `./llms-assurance/freshness-config*` subpaths are gone, replaced by the deep-frozen `LLM_FRESHNESS_CONFIG` constant exported from `./llms-assurance` with byte-identical values -- `audits/__tests__/llms-spoke-evidence.test.ts` pins the stamped `source.repository` to that constant.
  - **The run-source builders became exports at `0.9.0`; adoption here is deferred on purpose.** Atlas PR #268 (`0.8.0`) lifted the GitHub Actions run block out of `validateSpokeEvidence`'s `$.source` into the module-private `runSource` so a served verification's `$.verifiedBy` could ask the same rule; `0.9.0` (decision 0111 phase 3a, 0117 option 1D) promoted that seam to exported `workflowPath(value)` and `buildRunSource(fields)` on `./llms-assurance`, and atlas adopted both in the same release. This repo still builds locally: `checkedSource` in `audits/lib/llms-spoke-evidence.ts` constructs and type-narrows the envelope, and the validation half is already delegated -- `audits/checks/b2-check-llms-coherence.mjs` runs the published `assertSpokeEvidence` over every envelope before writing it. The `0.9.0` pin required no adoption (atlas PR #284's consumer map lists none for this repo); swapping `checkedSource` / `b2EvidenceSourceFromEnvironment` onto `buildRunSource` is a candidate follow-up, not part of the pin.
